...
r = redis.Redis(
    host="redis-12897.c12.us-east-1-4.ec2.redns.redis-cloud.com",
    port=12897,
    db=0,
    password='9bXWnQjto13hsEmheB1cgCQucIEH8NLS'
)

def get_student_from_db(student_name):
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
    cursor.execute("SELECT name, addr, city, zip FROM students WHERE name = ?", (student_name,))
...
# Rota para obter estudante
@app.route('/student/<student_name>', methods=['GET'])
def get_student(student_name):
    cache_key = f"student_{student_name}"
    cached_student = r.get(cache_key)
    
    if cached_student:
        student = json.loads(cached_student.decode('utf-8'))
        print('Pegando do Redis os dados cacheados!')
    else:
        student = get_student_from_db(student_name)
        if student:
            r.set(cache_key, json.dumps(student))
            r.expire(cache_key, 25)  # Cache expira em 25 segundos
            print('Cacheando os dados do SQLite no Redis!')
        else:
